...
contents=leaf_4|leaf_5
    ...
    def test_getObject_01(self):
        node_ = DC('node_2')
        node = self.nav.getObject('node_2')
    ...
    def test_isNode_01(self):
        self.assertEqual(self.nav.isNode(DC('root')), 1)

    def test_isNode_02(self):
        self.assertEqual(self.nav.isNode(DC('node_2')), 1)

    def test_isNode_03(self):
        self.assertEqual(self.nav.isNode(DC('leaf_1')), 0)

    def test_isNode_04(self):
        self.assertEqual(self.nav.isNode(DC('leaf_3')), 0)

    def test_getParent_01(self):
        current = DC('node_1')
        parent = self.nav.getParent(current)
    ...
    def test_getChildren_01(self):
        self.assertEqual(self.nav.getChildren(DC('root')),
                         [DC('node_1'),
                          DC('node_2'),
                          DC('leaf_1')])

    def test_getChildren_02(self):
        self.assertEqual(self.nav.getChildren(DC('node_3')),
                         [DC('leaf_4'), DC('leaf_5')])

    def test_getChildren_03(self):
        self.assertEqual(self.nav.getChildren(DC('leaf_1')), [])

    def test_getChildren_04(self):
        # XXX ConfNavigation should raise KeyError for invalid key
        self.assertEqual(self.nav.getChildren(DC('foo')), [])
